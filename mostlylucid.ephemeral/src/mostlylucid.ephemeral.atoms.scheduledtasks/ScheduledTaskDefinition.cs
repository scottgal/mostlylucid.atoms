...
public sealed class ScheduledTaskDefinition
{
    ...
    public static ScheduledTaskDefinition Parse(JsonElement element)
    {
        ...
        var key = element.TryGetProperty("key", out var keyValue) && keyValue.ValueKind == JsonValueKind.String ? keyValue.GetString() : null;
        var description = element.TryGetProperty("description", out var descValue) && descValue.ValueKind == JsonValueKind.String ? descValue.GetString() : null;
        var payload = element.TryGetProperty("payload", out var payloadValue) ? payloadValue.Clone() : (JsonElement?)null;
        var timezoneId = element.TryGetProperty("timeZone", out var tzValue) && tzValue.ValueKind == JsonValueKind.String ? tzValue.GetString() : null;
        var runOnStartup = element.TryGetProperty("runOnStartup", out var runValue) && runValue.ValueKind == JsonValueKind.True;
        var format = element.TryGetProperty("format", out var formatValue) && formatValue.ValueKind == JsonValueKind.String
            ? Enum.Parse<CronFormat>(formatValue.GetString() ?? string.Empty, ignoreCase: true)
        ...
        if (!string.IsNullOrEmpty(timezoneId))
        {
            ...
            {
                throw new InvalidOperationException($"Unknown timezone '{timezoneId}' for scheduled task '{name}'.", ex);
            }
        }
    ...
    private static string GetRequiredString(JsonElement element, string propertyName)
    {
        ...
        {
            throw new InvalidOperationException($"Scheduled task property '{propertyName}' is required and must be a string.");
        }
