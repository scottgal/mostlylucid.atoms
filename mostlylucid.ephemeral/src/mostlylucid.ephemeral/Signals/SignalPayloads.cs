    ...
    SignalPropagation? Propagation = null)
{
    ...
    public SignalEvent WithoutPayload()
    {
        return new SignalEvent(Signal, OperationId, Key, Timestamp, Propagation);
    }
}
...
public sealed class TypedSignalSink<TPayload>
{
    ...
    public void Raise(SignalEvent<TPayload> evt)
    {
        ...
        Untyped.Raise(evt.WithoutPayload());
    ...
    {
        Raise(new SignalEvent<TPayload>(signal, EphemeralIdGenerator.NextId(), key, DateTimeOffset.UtcNow, payload,
            propagation));
    }
