...
namespace Mostlylucid.Ephemeral.Atoms.Taxonomy.Guard.Tests;

public class GuardAtomTests
{
    [Fact]
    public async Task DefaultContractMatchesKind()
    {
        ...
        await using var atom = new GuardAtom<int, int>(typed, Handler, maxBodyDuration: TimeSpan.FromSeconds(30));

        Assert.Equal(AtomKind.Guard, atom.Contract.Kind);
    ...
    public async Task DefaultOutputSignalMatchesKind()
    {
        ...
        var expected = "atom.guard.output";
    ...
    public async Task RunAsyncEmitsTypedSignalAndReturnsOutput()
    {
        ...
        await using var atom = new GuardAtom<int, int>(typed, Handler, maxBodyDuration: TimeSpan.FromSeconds(30), outputSignal: "guard.output");
        var result = await atom.RunAsync(5);

        Assert.Equal(6, result);
        var evt = await WaitForSignalAsync(tcs.Task);
        Assert.Equal("guard.output", evt.Signal);
    ...
    public async Task KeySelectorIsUsedForOutputSignals()
    {
        ...
        await using var atom = new GuardAtom<int, int>(
            typed,
            Handler,
    ...
    public async Task EmitOutputSignalsFalseSuppressesEmission()
    {
        ...
        await using var atom = new GuardAtom<int, int>(typed, Handler, maxBodyDuration: TimeSpan.FromSeconds(30), emitOutputSignals: false);
